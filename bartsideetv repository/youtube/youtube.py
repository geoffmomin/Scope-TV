...
class Module(object):
    ...
    def Episode(self, show_name, show_id, page, totalpage):
        ...
        for info in soup.findAll('media:content', {'isdefault' : 'true'}):
            tmp_id = re.compile('v\/(.*?)\?', re.DOTALL + re.IGNORECASE).search(info['url']).group(1)
            path.append(tmp_id)
...
def YouTube(id):
    ...
    html = ba.FetchUrl(url, 0)
    regex = re.compile('fmt_url_map=([^&]+)').search(html).groups()
    fmt_raw = regex[0]
    fmt_set = fmt_raw.split('%2C')
    formats = {
        '17': 'mp4',
        '18': 'mp4',
        '22': 'mp4',
    ...
    for item in fmt_set:
        if not result:
            item = item.split('%7C')
            if item[0] in formats.keys():
                result = unquote_plus(item[1])
                ext = formats[item[0]]
    return result, ext